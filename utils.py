...
def call_model(prompt, api_key=None, use_local_model=False):
    use_local_model = False  # Deshabilitado temporalmente
    if use_local_model:
        ...
        # Configuración de la clave API de Hugging Face
        headers = {
        ...
        # Procesar la respuesta JSON y verificar que tiene el formato esperado
        json_response = response.json()
        
        # Verifica si la respuesta es una lista no vacía y contiene "generated_text"
        if isinstance(json_response, list) and len(json_response) > 0 and "generated_text" in json_response[0]:
            return clean_text(json_response[0]["generated_text"])
        else:
            print("Error: La respuesta no tiene el formato esperado:", json_response)
            return ""
...
def generate_image(prompt, output_file, api_token, max_retries=5, wait_time=60):
    """
    Generate an image using the Hugging Face API with retry logic for rate limits.
    ...
    - wait_time: Time to wait (in seconds) before retrying after a rate limit error.
    """
    ...
    while retries < max_retries:
        ...
        if response.status_code == 200 and response.headers['Content-Type'].startswith('image'):
            ...
            print(f"Error 429: Rate limit reached. Retrying in {wait_time} seconds...")
            retries += 1
            ...
            return False  # Exit the function on non-rate-limit errors
